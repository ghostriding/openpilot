...
class LatControl(object):
  ...
  def update(self, active, v_ego, angle_steers, steer_override, d_poly, angle_offset, CP, VM, PL):
    ...
    if self.last_mpc_ts < PL.last_md_ts:
      ...
      if abs(self.angle_Check) < 0.5:                 # 0.5 degrees
        self.variableSteerRatio = self.lowSteerRatio  # Use the lower ratio
      elif 0.5 < abs(self.angle_Check) < 1.5:         # The VSR transition zone
        # (a + 0.5) * sratio = magic (adj ratio)
        self.variableSteerRatio = (self.angle_Check + 0.5) * self.lowSteerRatio  # This is the _variable_ part
